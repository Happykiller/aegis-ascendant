...
- [Vérifier un rendu visuel depuis WSL](howto-verifier-un-rendu.md) — capture PNG autonome, sans
  ...
  `--capture-after` compte des **images**, pas des secondes. ⚠️ **Un `--skip-to-*` seul ne quitte pas l'écran-titre** : il est lu par le NIVEAU, qui n'est pas chargé — il faut `--goto-graybox` devant. Symptôme : `[TitleStage] ready` sans `[Level] ready`, sortie en `code 0`, un lancement complet pour rien. ⚠️ **Ce qui doit rester discret doit être FIN, pas transparent** : le bloom et le `lift` du post-traitement ravivent toute grande surface teintée, même à 11 % d'opacité. ⚠️ **LE TÉMOIN AVANT L'HYPOTHÈSE** : quand un artefact apparaît sur UN sujet, capturer un AUTRE sujet — pas théoriser. Trois causes fausses proposées sur une tache du bestiaire, un cycle chacune ; une capture d'une autre fiche a tranché en trente secondes. ⚠️ **Un `grep` dans le tuyau avale une porte ROUGE** : le code de retour est celui de la dernière commande, l'export tourne sur les sources précédentes, et la capture obtenue est identique **au MD5 près** à la précédente. ⛔ **VÉRIFIER LA CHAÎNE D'ÉCHANTILLONNAGE AVANT DE RÉGLER CE QUI LA TRAVERSE** (13/09) : onze textures jugées « moches, du bruit de télé » — elles étaient importées **sans mipmaps**, donc échantillonnées point par point à 19× de réduction. J'ai réglé une échelle, puis un relief, puis l'échelle de nouveau SUR CE SIGNAL CASSÉ, et les trois conclusions se sont inversées une fois la chaîne saine. **Coût : une soirée, et « en fait, j'en ai marre ».** ⚠️ La leçon était DÉJÀ écrite dans [howto-assets-image-genere](howto-assets-image-genere.md) : elle est devenue une **règle dure** de `lint-regles.sh`, avec sa dette gelée. ⚠️ Et **le contraste local ne voit pas une texture** — trois mensonges dans le même sens le même jour : il est dominé par les contrastes GÉOMÉTRIQUES. Il juge une silhouette, jamais une surface.
...
| `blender-aegis -b -P tools/render-hull.py -- <glb>` | intégrer une coque 3D **sans l'avoir regardée** (planche 4 vues, dont l'angle réel de la caméra de jeu) |
